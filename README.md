...
However this software is only one part of the equation. To record a useful dataset a text sentence corpus also needs to be acquired with a meaningful distribution of sentences. For this purpose I am also working on a sentence extractor that can take basically any large text files as input and extract sentences of varying length from it.

With that dataset acquired it is then a straight-forward, but timeconsuming, process of using SentRec to record all of the sentences. But SentRec will help you ease that process by providing a few-clicks workflow that includes normalizing and cutting silence from the recorded sentences. In other words, the output will be directly usable to train a Piper TTS voice.